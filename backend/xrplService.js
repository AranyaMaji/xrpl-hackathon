...
exports.initLedger = async () => {
    ...
    console.log(`[XRPL: SUCCESS] Parker funded: ${f1.wallet.address} (Balance: ${f1.balance} XRP)`);
    
    console.log("[XRPL: EVENT] Funding Owner Wallet...");
    const f2 = await state.client.fundWallet();
    console.log(`[XRPL: SUCCESS] Owner funded: ${f2.wallet.address} (Balance: ${f2.balance} XRP)`);
    
    console.log("[XRPL: EVENT] Funding Admin Wallet...");
    const f3 = await state.client.fundWallet();
    console.log(`[XRPL: SUCCESS] Admin funded: ${f3.wallet.address} (Balance: ${f3.balance} XRP)`);
...
exports.startParkingOnChain = async (spot) => {
    ...
    console.log(`[XRPL: ACTION] Submitting PaymentChannelCreate (${maxPotentialDrops} drops)...`);
    ...
    console.log(`[XRPL: RESPONSE] PaymentChannel TX Result: ${payChanTx.result.meta.TransactionResult}`);
    
    // STRICT VALIDATION
    if (payChanTx.result.meta.TransactionResult !== "tesSUCCESS") {
        console.error("[XRPL: FATAL ERROR] Payment Channel Failed:", payChanTx.result);
        throw new Error(`XRPL Rejected Channel: ${payChanTx.result.meta.TransactionResult}`);
    }
    ...
    if (!createdNode) throw new Error("PayChannel node not found in ledger metadata!");
    spot.channelId = createdNode.CreatedNode.LedgerIndex;
    console.log(`[XRPL: SUCCESS] Channel ID mapped: ${spot.channelId}`);

    console.log(`[XRPL: ACTION] Submitting EscrowCreate (${collateralDrops} drops)...`);
    ...
    console.log(`[XRPL: RESPONSE] EscrowCreate TX Result: ${escrowTx.result.meta.TransactionResult}`);
    
    if (escrowTx.result.meta.TransactionResult !== "tesSUCCESS") {
         console.error("[XRPL: FATAL ERROR] Escrow Failed:", escrowTx.result);
         throw new Error(`XRPL Rejected Escrow: ${escrowTx.result.meta.TransactionResult}`);
    }

    console.log(`[XRPL: SUCCESS] On-Chain setup entirely complete.\n`);
    return { channelHash: payChanTx.result.hash, escrowHash: escrowTx.result.hash };
};

exports.claimHourlyRate = async (spot) => {
    // Prevent claiming more than the channel holds
    if (spot.hoursElapsed > spot.duration) {
        console.log(`[XRPL: DAEMON] Max duration reached for ${spot.id}. Halting trickle.`);
        return spot.lastTrickleHash;
    }

    console.log(`[XRPL: DAEMON] Generating off-chain signature for Hour ${spot.hoursElapsed}...`);
    const amountToClaim = (spot.price * spot.hoursElapsed * 1000000).toString();
    const signature = xrpl.authorizeChannel(state.parkerWallet, spot.channelId, amountToClaim);

    console.log(`[XRPL: DAEMON] Submitting PaymentChannelClaim for ${amountToClaim} drops...`);
    ...
    console.log(`[XRPL: DAEMON SUCCESS] Trickle complete. Hash: ${claimTx.result.hash}`);
...
exports.slashCollateral = async (spot) => {
    ...
    console.log(`[XRPL: RESPONSE] EscrowFinish Result: ${finishTx.result.meta.TransactionResult}\n`);
