...
let spots = [
    { id: 'unsw-001', lat: -33.9155, lng: 151.228, address: '14 High St, Kensington', price: 3.0, status: 'AVAILABLE', duration: 0, minutesElapsed: 0, votes: {} },
    { id: 'unsw-002', lat: -33.9161, lng: 151.234, address: '55 High St, Randwick', price: 3.5, status: 'AVAILABLE', duration: 0, minutesElapsed: 0, votes: {} },
    { id: 'unsw-003', lat: -33.9115, lng: 151.2335, address: '12 Clara St, Randwick', price: 1.5, status: 'AVAILABLE', duration: 0, minutesElapsed: 0, votes: {} },
    { id: 'unsw-004', lat: -33.918, lng: 151.2375, address: '12 Botany St, Randwick', price: 2.0, status: 'AVAILABLE', duration: 0, minutesElapsed: 0, votes: {} },
    { id: 'unsw-006', lat: -33.9215, lng: 151.235, address: '110 Barker St, Kingsford', price: 3.5, status: 'AVAILABLE', duration: 0, minutesElapsed: 0, votes: {} },
    { id: 'unsw-010', lat: -33.9185, lng: 151.2245, address: '150 Anzac Pde, Kensington', price: 4.5, status: 'AVAILABLE', duration: 0, minutesElapsed: 0, votes: {} },
];
...
setInterval(async () => {
    for (let spot of spots) {
        if (spot.status === 'ACTIVE') {
            ...
            try {
                spot.lastTrickleHash = await xrplService.claimFractionalRate(spot);
            } catch (e) {
                console.error(`[CRON ERROR] Trickle failed on spot ${spot.id}:`, e.message);
            }

            // Duration is in hours, so multiply by 60 to compare with minutes
            if (spot.minutesElapsed >= spot.duration * 60) {
                console.log(`[CRON] ⏰ Spot ${spot.id} duration expired! Awaiting consensus votes.`);
                spot.status = 'EXPIRED';
            }
        }
    }
...
app.post('/api/vote', async (req, res) => {
    ...
    const processReputation = (status) => {
        if (status === 'RESOLVED_SAFE') {
            ...
            userProfiles.parker.trustScore = Math.min(850, userProfiles.parker.trustScore + 25);
            userProfiles.parker.safeCheckouts++;

            if (userProfiles.parker.trustScore >= 700 && !userProfiles.parker.hasNFT) {
                userProfiles.parker.hasNFT = true;
                spot.nftMinted = true;
            }
        } else if (status === 'RESOLVED_PENALTY') {
            // -150 Points for slashing, min 300
            userProfiles.parker.trustScore = Math.max(300, userProfiles.parker.trustScore - 150);
    ...
    if (votesArr.filter((v) => v === 'penalty').length >= 2 || votesArr.filter((v) => v === 'safe').length >= 2) {
        const isPenalty = votesArr.filter((v) => v === 'penalty').length >= 2;
        spot.status = isPenalty ? 'RESOLVED_PENALTY' : 'RESOLVED_SAFE';
        processReputation(spot.status);
        if (isPenalty)
            try { spot.escrowHash = await xrplService.slashCollateral(spot); } catch (e) {}
    } else if (spot.votes.parker && spot.votes.owner && spot.votes.parker !== spot.votes.owner) {
